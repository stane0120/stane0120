...
COMMENT ON COLUMN "Untitled3"."COMMENT_NO" IS '댓글 번호(시퀀스 SEQ_COMMENT_NO)';

COMMENT ON COLUMN "Untitled3"."COMMENT_CONTENT" IS '댓글 내용';

COMMENT ON COLUMN "Untitled3"."CREATE_DT" IS '댓글 작성일';

COMMENT ON COLUMN "Untitled3"."MEMBER_NO" IS '회원번호 (FK)';

COMMENT ON COLUMN "Untitled3"."BOARD_NO" IS '게시글번호(FK)';
...
WHERE BOARD_NO != -- INDEX 사용
ORDER BY CREATE_DT DESC;


-- 댓글 샘플 데이터 삽입
INSERT INTO "Untitled3"
...
SELECT BOARD_NO, BOARD_TITLE, MEMBER_NM, CREATE_DT, READ_COUNT,
       (SELECT COUNT(*) FROM "COMMENT" SUB
        WHERE SUB_BOARD_NO = MAIN.BOARD_NO
