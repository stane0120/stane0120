SELECT DEPT_CODE FROM EMPLOYE; -- 23행 조회
...
FROM EMPLOYEE
--WHERE SUBSTR(EMP_NO, 1, 2) >= 70, SUBSTR(EMP_NO,1,2) < 80
--WHERE SUBSTR(EMP_NO, 1, 2) BETWEEN '70' AND '79'
--WHERE SUBSTR(EMP_NO,1,2) LIKE '7%'
WHERE SUBSTR(EMP_NO,1,1) '7'
--WHERE SUBSTR(EMP_NO, 1, 2) BETWEEN '700101' AND '791231';
GROUP BY DEPT_CODE
...
-- UNION ALL 확인
-- 부서코드가 'D5' 또는 급여 300만 초과인 사원 조회 (쭝복 O)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE 
WHERE DEPT_CODE = 'D5'
UNION ALL
...
-- MINUS 확인
-- 부서코드가 'D5' 이지만 300만 초과인 사원 제외한 후 조회 
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE 
WHERE DEPT_CODE = 'D5'
MINUS
...
-- 집합 연산은 2개 이상의 SELECT 문에 사용 가능하다
