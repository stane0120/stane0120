...
CREATE USER bjh_sample IDENTIFIED BY "sample1234";
                                           --> 대소문자 구분

-- 2. 생성된 sample 계정 접속 방법 추가
   --> CREATE SESSION (접속 권한)이 없음
   --> sample 계정에 접속 관한을 부여(GRANT)

-- 3. 접속 권한 부여
-- GRANT 권한 [, 권한,....] TO 계정명;
GRANT CREATE SESSION TO bjh_sample;
...
-- 6. 테이블 생성 권한 부여
--   + 객체 생성 공간 할당
GRANT CREATE TABLE TO bjh_sample;

ALTER USER bjh_sample DEFAULT TABLESPACE SYSTEM
...
GRANT SELECT ON EMPLOYEE TO bjh_sample;
...
-- 4. 선동일 삭제
DELETE FROM KH_BJH.EMPLOYEE
WHERE EMP_ID = 200;
-- ORA-01031: 권한이 불충분합니다

--------------------------- kh 계정으로 접속 -------------------------------
-- 5. sample 계정의 EMPLOYEE 테이블 조회 권한 회수
REVOKE SELECT ON EMPLOYEE FROM bjh_sample;
...
GRANT CREATE SEQUENCE,
      ...
      CREATE TYPE,
      CREATE TRIGGER,
      CREATE TABLE TO bjh-sample;

--> ROLE을 이용해서 간단히 부여 / 회수
GRANT RESOURCE TO bjh_sample;
REVOKE RESOURCE FROM bjh_sample;

-- 사용자에게 부여된 시스템권한 조회
-- SELECT * FROM DBA_SYS_PRIVS
-- WHERE GRANTEE = 'BJH_SAMPLE';
