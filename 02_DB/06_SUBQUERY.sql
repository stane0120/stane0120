...
WHERE SALARY >= (SELECT AVG(SALARY) FROM EMPLOYEE) -- 3047633 
ORDER BY JOB_CODE;
...
--          메인 쿼리에 하나라도 있으면 TRUE(조회 결과에 포함)
--          --> 서브쿼리 조화 결과가 1행 이상이면 메인 쿼리 행을 결과에 포함
