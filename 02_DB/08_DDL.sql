...
-- DEFAULT(기본값) :
-- INSERT/UPDATE 시 해당 컬럼의 값을 작성하지 않거나
-- DEFAULT로 작성하면
...
SELECT * FROM MEMBER;
SELECT * FROM USER_TABLES;
     --> 딕셔너리 뷰(데이터 딕셔너리에서 일부만 뽑아 보여주는 뷰)
...
-- ORA-00001: 무결성 제약 조건(KH_BJH.SYS_C007464)에 위배됩니다
-- ORA-00001: 무결성 제약 조건(KH_BJH.USER_ID_UK)에 위배됩니다
...
INSERT INTO USER_USED_UK2
VALUES(1, 'user01', 'pass01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_BJH.USER_ID_NAME_UK)에 위배됩니다
...
CREATE TABLE USER_USED_PK(
 --   USER_NO NUMBER PRIMARY KEY, -- 컬럼 레벨(제약조건명 )
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(KH_BJH.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_BJH"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(KH_BJH.PK_USERNO_USERID)에 위배됩니다
...
-- ORA-01400: NULL을 ("KH_BJH"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
--> 복합키로 PK가 설정되어 있어도
-- 결국은 PK == NULL 삽입 불가능
...
-- 컬럼명 자료형(크기) [CONSTRAINT 이름] REFERENCES 참조할 테이블명 [(참조할 테이블명)]
...
-- ORA-02291: 무결성 제약조건(KH_BJH.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
...
DELETE FROM USER_GRADE
WHERE GRADE_CODE = 10;
-- ORA-02292: 무결성 제약조건(KH_BJH.GRADE_CODE_FK1)이 위배되었습니다
-- 자식 레코드가 발견되었습니다
...
-- ORA-02290: 체크 제약조건(KH_BJH.GENDER_CHECK)이 위배되었습니다
...
ALTER TABLE EMPLOYEE CONSTRAINT FK_JOB_CODE
